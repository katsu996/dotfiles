...
PR のブランチをローカルにチェックアウトし、最新の状態にする。
**いずれかのコマンドが失敗した場合は、エラーメッセージを表示して処理を中断する。**

```bash
# ステップ1: PRブランチをチェックアウト（ローカルに存在しない場合も自動で追跡ブランチを作成）
...
### B-0. ブランチ切り替えとプル（MD ファイルパス引数の場合のみ）

引数が MD ファイルパスの場合、ファイル名から PR 番号を抽出してブランチを切り替える。
**いずれかのコマンドが失敗した場合は、エラーメッセージを表示して処理を中断する。**

ファイル名の命名規則: `pr{PR_NUMBER}_{YYYYMMDDHHmm}_{連番}_{名前}.md`

```bash
# ファイル名先頭の pr の直後の数字を PR 番号として抽出
BASENAME=$(basename {MDファイルパス})
PR_NUMBER=$(echo "$BASENAME" | sed -E 's/^pr([0-9]+)_.*/\1/')

# 抽出できなかった場合はエラーで中断
if [ -z "$PR_NUMBER" ]; then
  echo "エラー: ファイル名から PR 番号を抽出できませんでした: $BASENAME"
  # 処理を中断
fi
